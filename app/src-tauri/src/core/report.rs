...
fn generate_markdown(summary: &SummaryResult, journals: &[JournalCandidate]) -> String {
    ...
    lines.push("# 投稿先選定レポート — Journal Finder 論文投稿先アドバイザ".to_string());
    ...
    lines.push(String::new());
    lines.push("---".to_string());
    lines.push("*Generated by Journal Finder 論文投稿先アドバイザ*".to_string());
...
fn generate_docx_base64(summary: &SummaryResult, journals: &[JournalCandidate]) -> String {
    ...
    doc = doc.add_paragraph(
        Paragraph::new()
            .add_run(Run::new().add_text("投稿先選定レポート — Journal Finder 論文投稿先アドバイザ").bold().size(28)),
    );
